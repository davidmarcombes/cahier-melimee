...
if (!inputPath) {
  console.error('Usage: node scripts/image-to-magic-color.js <image.png> [--cols=8] [--rows=8] [--max-colors=4] [--output=out.md] [--repeat=3] [--rule=direct]');
  process.exit(1);
}
...
while (palette.length > MAX_COLORS) {
  ...
  palette[mergeA].r = Math.round((palette[mergeA].r * (total - palette[mergeB].count) + palette[mergeB].r * palette[mergeB].count) / total);
  palette[mergeA].g = Math.round((palette[mergeA].g * (total - palette[mergeB].count) + palette[mergeB].g * palette[mergeB].count) / total);
  palette[mergeA].b = Math.round((palette[mergeA].b * (total - palette[mergeB].count) + palette[mergeB].b * palette[mergeB].count) / total);
...
const labelsYaml = FORCE_RULE === 'direct'
...
if (FORCE_RULE === 'direct') {
  ...
  const rangesYaml = paletteHex.map((_, i) => {
