...
for (const { dir: scanRoot, kind } of SCAN) {
    for (const seriesDir of findSeriesDirs(scanRoot)) {
        ...
        let meta = {};
        try { meta = yaml.load(fs.readFileSync(indexPath, 'utf8')) || {}; } catch {}
        ...
        const mdFiles = fs.existsSync(seriesDir)
            ? fs.readdirSync(seriesDir).filter(f => f.endsWith('.md')).map(f => path.join(seriesDir, f))
            : [];
...
rows.sort((a, b) => {
    ...
    const dA = DIFF_ORDER[a.difficulty] ?? 99, dB = DIFF_ORDER[b.difficulty] ?? 99;
...
const COLUMNS = [
    'kind', 'path', 'id', 'seriesTitle', 'level', 'subject', 'topic',
    'difficulty', 'exerciseCount', 'repeatTotal', 'types', 'generators',
];
...
const C = { bold: '\x1b[1m', dim: '\x1b[2m', cyan: '\x1b[36m', yellow: '\x1b[33m', green: '\x1b[32m', reset: '\x1b[0m' };
