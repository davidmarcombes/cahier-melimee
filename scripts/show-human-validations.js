...
console.log(
  `  Series    : ${C.green}${fullySeries} fully${C.reset}  ${C.yellow}${partialSeries} partial${C.reset}  ${C.dim}${unstartedSeries} not started${C.reset}  (${totalSeries} total)`
);
...
console.log('SERIES ID'.padEnd(colW.id) + 'HASH'.padEnd(colW.hash) + 'FILE'.padEnd(colW.path) + 'VALIDATED AT');
console.log(LINE);
...
console.log(LINE);
console.log(
  `Total: ${C.bold}${rows.length}${C.reset} file validations across ${C.bold}${totalSeries}${C.reset} series\n`
);
