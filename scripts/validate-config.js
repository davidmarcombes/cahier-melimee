...
function checkTemplateFiles() {
  ...
  const templateFiles = ['SITE.md.template', 'src/_data/site.json.template', 'src/_data/navigation.json.template'];
...
if (issues.length > 0) {
  ...
  if (criticalIssues.length > 0) {
    console.log(
      `${colors.red}❌ Found ${criticalIssues.length} placeholder(s) in critical configuration files. Please update these before deployment.${colors.reset}\n`
    );
    process.exit(1);
  } else {
    console.log(
      `${colors.cyan}ℹ️ Non-critical placeholders were found (documentation or example content). These do not block validation.${colors.reset}\n`
    );
    process.exit(0);
  }
