...
const TYPE_SCHEMAS = {
  'number-check': {
    required: [],
    requireOneOf: [['answer'], ['answers'], ['generator']],
    desc: 'Simple operation with answer input (trou / generator supported)',
  },
  problem: {
    required: [],
    requireOneOf: [['answer'], ['answers'], ['generator']],
    desc: 'Word problem — markdown body is the story, answer is numerical/text',
  },
  ...
  'logic-grid': {
    required: ['columns', 'rows', 'solution'],
    desc: 'Logic grid — columns[], rows[], solution{row:col}',
  },
  ...
  'multi-question': {
    required: ['questions'],
    desc: 'Context + sub-questions — questions[].text + questions[].answer; markdown body = scenario',
  },
  ...
  'base-10': {
    required: ['answer'],
    requireOneOf: [['number'], ['hundreds', 'tens', 'ones']],
    desc: 'Base-10 blocks — decompose hundreds/tens/ones',
  },
  ...
  'fill-table': {
    required: ['headers', 'rows', 'answers'],
    desc: 'Table with blanks — ? marks the blanks, answers[] lists values',
  },
  checkbox: {
    required: ['statements', 'checkedAnswers'],
    desc: 'Tick true statements — checkedAnswers[] = 0-indexed correct indices',
  },
  select: {
    required: ['choices', 'statements'],
    desc: 'Dropdown completion — choices[] + statements[].template (___) + .answer',
  },
  'svg-tiles': {
    required: ['tiles', 'answers'],
    desc: 'SVG tile grid — tiles[].gen + tiles[].par, answers[] = 0-indexed',
  },
  'tile-select': {
    required: ['tiles', 'tileAnswers'],
    desc: 'Click correct tiles — tiles[] (HTML strings), tileAnswers[] = 0-indexed',
  },
  'fraction-check': {
    required: [],
    requireOneOf: [['answer'], ['answers']],
    desc: 'Fraction input — answers[numerator, denominator] or answer: "n/d"',
  },
  ruler: { required: [], desc: 'Graduated ruler — min, max, divisions, markers[].label + .value; answer = value read' },
  'click-blocks': {
    required: ['columns'],
    desc: 'Place-value blocks — columns[].label, .value, .color, .answer, .max',
  },
  'number-line': { required: ['min', 'max', 'answer'], desc: 'Number line — place or read a value; step optional' },
  'coordinate-grid': {
    required: ['answer'],
    desc: 'Grid read/place — mode: read|place, cols, rows, points[].x/y/label',
  },
  'bar-chart': { required: ['labels', 'values', 'yMax', 'yStep'], desc: 'Bar chart — labels[], values[], axis config' },
  'calc-chain': {
    required: ['start', 'steps'],
    desc: 'Calc chain — start value + steps[].op + steps[].value + steps[].answer',
  },
  ...
  'column-op': {
    required: ['top', 'operation', 'result'],
    desc: 'Vertical operation — top[], operation (+/-/×/÷), result; NOT wired into series-player',
  },
};
