...
if (!inputPath) {
  console.error('Usage: node scripts/pixelate-image.js <input> [--cols=12] [--rows=12] [--colors=6] [--output=out.png]');
  process.exit(1);
}
...
(async () => {
  ...
  console.log(`  node scripts/image-to-magic-color.js ${OUTPUT} --cols=${COLS} --rows=${ROWS} --max-colors=${NCOLORS} --rule=direct`);
})();
